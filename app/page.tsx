...
const services = [
  { name: 'Website Development', desc: 'Fast, responsive sites built with modern frameworks. SEO-friendly and designed for conversions.', icon: '🌐' },
  { name: 'AI Solutions', desc: 'Custom AI integration for automation, chatbots, and intelligent workflows tailored to your needs.', icon: '🤖' },
  { name: 'SaaS Development', desc: 'Full-stack SaaS platforms from concept to launch with scalable architecture and clean code.', icon: '⚡' },
  { name: 'Mobile Apps', desc: 'Native and cross-platform apps that deliver great performance and user experience on iOS and Android.', icon: '📱' },
  { name: 'UI/UX Design', desc: 'Beautiful, intuitive interfaces that drive engagement and conversions for your digital product.', icon: '🎨' },
  { name: 'Cloud & Automation', desc: 'AWS, GCP, and Azure deployment with CI/CD pipelines and infrastructure optimization.', icon: '☁️' }
];
...
function JivraLogo({ className = '' }: { className?: string }) {
  return (
    <a id="home" href="#about" className={`inline-flex items-center gap-2 transition-opacity hover:opacity-80 ${className}`}>
      {/* Icon — flat indigo rounded square, matching reference */}
      <span
      ...
      <div className="mx-auto max-w-7xl px-4 pb-16 pt-6 sm:px-6 sm:pb-28 sm:pt-10 lg:px-10">
        ...
        <AnimatePresence>
          {menuOpen && (
            ...
            >
              {['#services', '#process', '#testimonials', '#contact'].map((href) => (
                <a
                  key={href}
                  href={href}
                  onClick={() => setMenuOpen(false)}
                  className="block rounded-xl px-4 py-3 text-sm text-slate-300 transition hover:bg-white/5 hover:text-white capitalize"
                >
                  {href.replace('#', '')}
                </a>
              ))}
        ...
        <section className="relative mt-12 sm:mt-16 grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          ...
          >
            ...
            <div className="flex flex-col xs:flex-row flex-wrap gap-3 sm:gap-4">
              <a href="#contact" className="inline-flex items-center justify-center rounded-full bg-[#5f4cff] px-6 py-3 text-sm font-semibold text-white shadow-glow transition duration-300 hover:-translate-y-0.5 hover:bg-[#7d62ff] w-full xs:w-auto">
                Book a Call
              </a>
              <a href="#services" className="inline-flex items-center justify-center rounded-full border border-white/15 bg-white/5 px-6 py-3 text-sm font-semibold text-slate-100 transition duration-300 hover:border-[#7d62ff]/60 hover:text-white w-full xs:w-auto">
                View Services
              </a>
            </div>
          ...
          {/* Hero card — hidden on small screens to save space */}
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.9, ease: 'easeOut', delay: 0.1 }}
            className="relative hidden lg:block overflow-hidden rounded-[40px] border border-white/10 bg-white/5 p-8 shadow-soft backdrop-blur-xl"
          >
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {services.map((service) => (
              <motion.div
                ...
                className="group rounded-[24px] sm:rounded-[28px] border border-white/10 bg-[#0b0b13]/85 p-5 sm:p-6 transition duration-300"
              >
                <div className="mb-4 sm:mb-5 inline-flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-xl sm:rounded-2xl bg-[#1b133d] text-[#7d62ff] shadow-glow">
                  <span className="text-xl sm:text-2xl">{service.icon}</span>
                </div>
                <h3 className="text-base sm:text-lg font-semibold text-white">{service.name}</h3>
                <p className="mt-2 sm:mt-3 text-sm leading-6 text-slate-400">{service.desc}</p>
              </motion.div>
            ))}
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {processSteps.map((item) => (
              <motion.div
                key={item.step}
                whileHover={{ y: -4 }}
                className="group rounded-[28px] sm:rounded-[32px] border border-white/10 bg-[#0b0b13]/90 p-5 sm:p-6 shadow-soft transition"
              >
        ...
        <section id="why" className="mt-16 sm:mt-20 grid gap-5 sm:gap-6 lg:gap-10 lg:grid-cols-[0.7fr_1.3fr]">
          <div className="rounded-[32px] sm:rounded-[36px] border border-white/10 bg-[#08080f]/90 p-5 sm:p-8 lg:p-10 shadow-soft backdrop-blur-xl">
            ...
            <div className="mt-6 sm:mt-8 grid grid-cols-2 gap-2 sm:gap-3 lg:grid-cols-1">
              {reasons.map((reason) => (
                <div key={reason} className="rounded-2xl sm:rounded-3xl border border-white/10 bg-white/5 px-4 py-3 text-xs sm:text-sm text-slate-300">
                  {reason}
                </div>
              ))}
            </div>
          ...
          <div className="grid gap-4 sm:gap-5 grid-cols-1 sm:grid-cols-2">
            {[
              ...
              <motion.div
                key={card.label}
                whileHover={{ y: -4 }}
                className="rounded-[28px] sm:rounded-[32px] border border-white/10 bg-[#0b0a13]/90 p-5 sm:p-8 shadow-soft transition duration-300"
              >
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-5 md:grid-cols-2 lg:grid-cols-3">
            {testimonials.map((testimonial) => (
              <motion.div
                ...
                whileHover={{ y: -4 }}
                className="rounded-[28px] sm:rounded-[36px] border border-white/10 bg-[#09090f]/90 p-6 sm:p-8 shadow-soft backdrop-blur-xl"
              >
          ...
          <div className="grid gap-8 lg:gap-10 lg:grid-cols-[0.8fr_1.2fr]">
            <div className="space-y-5 sm:space-y-6">
              ...
              <div className="space-y-2 sm:space-y-3 text-sm text-slate-300">
                ...
                <p><strong className="text-white">WhatsApp:</strong> +91 98765 43210</p>
              </div>
              <div className="flex flex-wrap gap-2 sm:gap-3 text-sm text-slate-300">
                ...
                <a href="https://dribbble.com/jivra" target="_blank" rel="noopener noreferrer" className="rounded-full border border-white/10 bg-white/5 px-4 py-2 transition hover:bg-white/10">Dribbble</a>
              </div>
            </div>

            <form className="space-y-4 sm:space-y-5 rounded-[24px] sm:rounded-[32px] border border-white/10 bg-[#0b0b13]/95 p-5 sm:p-6 lg:p-8">
              <div>
                <label htmlFor="contact-name" className="mb-2 block text-sm font-medium text-slate-300">Name</label>
                <input id="contact-name" type="text" placeholder="Your name" className="w-full rounded-2xl sm:rounded-3xl border border-white/10 bg-[#0a0a12] px-4 py-3 text-sm text-white outline-none transition focus:border-[#7d62ff]/60 focus:ring-2 focus:ring-[#7d62ff]/20" />
              </div>
              <div>
                <label htmlFor="contact-email" className="mb-2 block text-sm font-medium text-slate-300">Email</label>
                <input id="contact-email" type="email" placeholder="you@email.com" className="w-full rounded-2xl sm:rounded-3xl border border-white/10 bg-[#0a0a12] px-4 py-3 text-sm text-white outline-none transition focus:border-[#7d62ff]/60 focus:ring-2 focus:ring-[#7d62ff]/20" />
              </div>
              <div>
                <label htmlFor="contact-message" className="mb-2 block text-sm font-medium text-slate-300">Project details</label>
                <textarea id="contact-message" rows={5} placeholder="Tell us about your vision" className="w-full rounded-[20px] sm:rounded-[28px] border border-white/10 bg-[#0a0a12] px-4 py-3 text-sm text-white outline-none transition focus:border-[#7d62ff]/60 focus:ring-2 focus:ring-[#7d62ff]/20 resize-none" />
              </div>
              <button type="submit" className="inline-flex w-full items-center justify-center rounded-full bg-[#5f4cff] px-6 py-3 text-sm font-semibold text-white shadow-glow transition hover:bg-[#7d62ff] active:scale-95">
                Send Message
              </button>
            </form>
          </div>
